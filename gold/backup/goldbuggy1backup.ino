...
// Command format:
//   S1n  -> start Buggy 1 with n loops, example: S13
//   R1   -> resume Buggy 1
// =====================================
...
void processCommand(String cmd) {
  // Start format: S1n, e.g. S13 means Buggy 1 start with 3 loops
  if (cmd.length() >= 3 && cmd.charAt(0) == 'S' && cmd.charAt(1) == '1') {
    if (state == WAIT_START) {
      ...
      state = EXIT_PARKING;
      stateStartTime = millis();
      sendMsg("<B1_START_" + String(targetLoops) + ">");
    }
  }
  else if (cmd == "R1") {
  ...
  if (gState == HIGH && !gantryLatched) {
    ...
    if (gId != 0) {
      ...
      if (!resumedOnce && gId == 3 && state == FOLLOW_TO_CENTER) {
        ...
        sendMsg("<B1_WAIT_CENTER>");
        return;
      }

      if (resumedOnce && gId == 2 && state == FOLLOW_AFTER_RESUME) {
        completedLoops++;
        sendMsg("<B1_LOOP_" + String(completedLoops) + ">");
