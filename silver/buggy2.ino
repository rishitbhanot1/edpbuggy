// =====================================
// BUGGY 2 - SILVER CHALLENGE
// Anticlockwise
// Starts only after coordinator sends S2
// Goes right first, then left
// At LEFT gantry, takes LEFT inward parking lane and parks
// =====================================
...
const unsigned long EXIT_LEFT_TIME    = 250;
  ...
  if (gState == HIGH && !gantryLatched) {
    ...
    if (gId != 0) {
      ...
      if (gId == 1) {
        ...
        sendMsg("<B2_G3_CENTER>");
      }

      // Buggy 2 parks when it reaches left gantry
      if (gId == 1 && state == FOLLOW_TO_LEFT_PARK) {
        leftParkArmed = true;
        sendMsg("<B2_G1_LEFT_PARK_ARM>");
      }
    }
  }
...
// ---------- LINE FOLLOW ----------
void followACW(int left, int right) {
  // keep same sensor mapping unless testing shows reversal is needed
  if (left == 1 && right == 1) {
    forward();
